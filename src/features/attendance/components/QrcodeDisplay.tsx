  ...
  return (
    <div className="relative z-[33] top-4 flex flex-col items-center">
      ...
      <button
        ...
        className={`w-full mx-auto gap-5 text-center flex items-center justify-center bg-blue-700 text-white py-2.5 rounded-md ${
          isDisabledButton ? 'hover:bg-blue-600' : 'hover:bg-blue-600'
      ...
      {localQrCodeData && (
        <div className="w-full flex flex-col gap-4">
          <button
            ...
            className={`w-full mt-4 mx-auto border border-dashed border-black dark:border-white gap-3 text-center flex items-center justify-center bg-transparent text-black dark:text-white py-3 rounded-md ${
              isDisabled || isDownloading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/10'
            } transition-colors duration-200`}
            disabled={isDisabled || isDownloading}
          >
            {isDownloading ? 'Downloading...' : 'Download QR Code'}
            <Download className="scale-[0.9]" />
          </button>
          <button
            onClick={toggleUploadModal}
            className="w-full py-3 rounded-md text-white text-lg bg-blue-700 hover:bg-blue-600 transition-colors duration-200"
          >
            Upload Audio
          </button>
          {(localStorage.getItem('statusFile') === 'processing' || summary || localStorage.getItem('audioSummary')) && (
            <button
              onClick={toggleSummaryModal}
              className="w-full py-4 rounded-md text-white bg-blue-700 hover:bg-blue-600 transition-colors duration-200"
            >
              Lihat hasil rangkuman
            </button>
          )}
        </div>
      )}
